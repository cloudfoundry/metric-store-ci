...
fly -t runway-ms sync || (
  echo "This requires the runway-ms target to be set"
  echo "Create this target by running 'fly -t runway-ms login -c https://runway-ci-sfo.eng.vmware.com -n phoenix-metric-store'"
  exit 1
)
